...
export function createAlert(
	...
	const validatedType = allowedTypes.includes(type.toLowerCase().trim())
	...
	// Check for a valid body
	if (Validator.isValidHtmlElement(footer)) {
		alert.append(footer);
	}
 ...
 */
export function createAlertHeader(
	text = "Alert Header",
	icon = {},
	options = {}
) {
	...
	if (!Validator.isString(text)) {
		console.warn(
			`Expected text to be a string, received ${typeof text}. Converting to string...`
		);
		text = text.toString();
	}
	...
	if (
		!Validator.isNonEmptyObject(icon) ||
		!Validator.isNonEmptyString(icon.name)
	)
		return header;

	const iconText = Validator.validateString(icon.name);
	if (!iconText) return header;
	const iconElement = createNewElement("i", {
		text: iconText,
		attributes: { class: icon.class || "material-icons" },
	});
	...
	if (
		Validator.isNonEmptyString(icon.position) &&
		icon.position.toLowerCase().trim() === "left"
	) {
...
export function createAlertBody(
	text = "Some random alert message",
	options = {}
) {
	// Check for a valid text value
	if (!Validator.isString(text)) {
